      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 bg-white p-4 sm:p-6 rounded-2xl border border-slate-200 shadow-xs">
        <div>
          ...
          <p className="text-xs text-slate-500 mt-1">
            기사별 근무 현황과 배정을 조회 및 변경할 수 있습니다.
          </p>
        </div>
        ...
        <div className="flex items-center justify-between sm:justify-start gap-2">
          ...
          <div className="flex items-center gap-1.5 sm:gap-2 px-2 sm:px-3 py-1.5 bg-slate-50 rounded-xl border border-slate-200 font-mono text-[10px] sm:text-xs font-bold text-slate-800 whitespace-nowrap min-w-0 overflow-hidden">
            ...
            <span className="truncate">
              {vm.dateColumns[0]?.dateStr?.slice(5)} ~{" "}
              {vm.dateColumns[vm.dateColumns.length - 1]?.dateStr?.slice(5)}
            </span>
          </div>
              ...
              <thead>
                <tr className="bg-slate-900 text-white text-xs font-bold divide-x divide-slate-800">
                  ...
                  {vm.dateColumns.map((col) => (
                    <th
                      key={col.dateStr}
                      className={`py-2.5 sm:py-3 px-2 sm:px-3 text-center min-w-[85px] sm:min-w-[100px] font-mono ${
                    ...
                    >
                      <td className="py-2.5 sm:py-3 px-3 sm:px-5 sticky left-0 bg-white group-hover:bg-slate-50 border-r border-slate-200 z-10 shadow-xs">
                        <div className="flex flex-col sm:flex-row items-start justify-between gap-1 sm:gap-2">
                          <div className="min-w-0">
                            ...
                            <div className="flex items-center gap-1 mt-0.5 flex-wrap">
                              {camps.map((c, i) => (
                                <span
                                  key={i}
                                  className="px-1.5 py-0.5 text-[10px] font-bold bg-slate-100 text-slate-700 rounded border border-slate-200"
                                >
                                  {c}
                                </span>
                              ))}
                          ...
                          <StatusBadge
                            status={row.contractType as any}
                            size="sm"
                          />
                        </div>
                      </td>
                      ...
                      {vm.dateColumns.map((col) => {
                        const shift = row.shifts[col.dateStr] || {
                          status: row.contractType as any,
                        };
                        return (
                          <td
                            ...
                            onClick={() =>
                              vm.setActiveCell({
                                ...
                                routeNumber: vm.getPrimaryRouteForRow(
                                  row,
                                  col.dateStr,
                                ),
                                date: col.dateStr,
                            ...
                            className={`py-2.5 sm:py-3 px-1.5 sm:px-2 text-center border-r border-slate-100 cursor-pointer hover:bg-blue-50/50 transition ${
