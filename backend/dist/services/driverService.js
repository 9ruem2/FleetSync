"use strict";
Object.defineProperty(exports, "__esModule", { value: true });
exports.driverService = void 0;
const driverRepository_1 = require("../repositories/driverRepository");
class DriverService {
    getAllDrivers(search, route, contractType) {
        let drivers = driverRepository_1.driverRepository.findAll();
        if (search && search.trim() !== '') {
            const q = search.trim().toLowerCase();
            drivers = drivers.filter(d => d.name.toLowerCase().includes(q) ||
                d.routeNumber.toLowerCase().includes(q) ||
                d.phone.includes(q));
        }
        if (route && route.trim() !== '') {
            drivers = drivers.filter(d => d.routeNumber === route.trim());
        }
        if (contractType && contractType.trim() !== '') {
            drivers = drivers.filter(d => d.contractType === contractType.trim());
        }
        return drivers;
    }
    getDriverById(id) {
        return driverRepository_1.driverRepository.findById(id) || null;
    }
    createDriver(dto) {
        if (!dto.name || !dto.phone || !dto.routeNumber || !dto.contractType) {
            throw new Error('필수 입력값이 누락되었습니다 (기사명, 연락처, 라우트번호, 계약형태)');
        }
        return driverRepository_1.driverRepository.create(dto);
    }
    updateDriver(id, dto) {
        const updated = driverRepository_1.driverRepository.update(id, dto);
        if (!updated) {
            throw new Error('해당 기사를 찾을 수 없거나 이미 삭제되었습니다');
        }
        return updated;
    }
    deleteDriver(id) {
        const success = driverRepository_1.driverRepository.softDelete(id);
        if (!success) {
            throw new Error('기사 삭제에 실패했습니다');
        }
        return true;
    }
}
exports.driverService = new DriverService();
