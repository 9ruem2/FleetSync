import { Driver, ScheduleShift, BackupAssignment } from '../types';
...
export function generateSeedShifts(drivers: Driver[]): ScheduleShift[] {
  ...
  for (let day = 1; day <= 31; day++) {
    ...
    drivers.forEach((driver) => {
      ...
      let status: '고정' | '용차' | '백업' | '휴무' = driver.contractType;

      // Add a few realistic off-days (휴무)
      if (
        (driver.id === 'drv-1' && (day === 12 || day === 18)) ||
...
export const initialBackupAssignments: BackupAssignment[] = [
  {
    id: 'bak-1',
    date: '2026-08-12',
    routeNumber: '101A',
